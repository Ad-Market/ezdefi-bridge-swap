...
function getContract(contractJson, address, {network}) {
  let web3
  if (network === 'foreign') {
    web3 = web3Foreign
    url = FOREIGN_RPC_URL
...
async function upgradeProxyAndCall({ proxy, implementationAddress, version, nonce, url, calldata }) {
  const data = await proxy.methods.upgradeToAndCall(version, implementationAddress, calldata).encodeABI()
  const sendTx = getSendTxMethod(url)
  const result = await sendTx({
    data,
    nonce,
