...
async function initializeBridge({ validatorsBridge, bridge, erc677token, initialNonce }) {
  ...
  if (isRewardableBridge) {
    ...
    const feeManager = await deployContract(FeeManagerErcToErc, [], {
    ...
    Home Fee: ${homeFeeInWei} which is ${HOME_TRANSACTIONS_FEE} in eth
    Foreign Fee: ${foreignFeeInWei} which is ${FOREIGN_TRANSACTIONS_FEE} in eth`)
...
async function deployHome() {
  console.log('COMMON_HOME_BRIDGE_ADDRESS: ', COMMON_HOME_BRIDGE_ADDRESS)
  const homeBridgeStorage = getContract(EternalStorageProxy, COMMON_HOME_BRIDGE_ADDRESS, { network: "home" })
  let currentVersion = await homeBridgeStorage.methods.version().call()

  console.log('[Home] HomeBridge implementation current version: ', currentVersion)
  ...
  const bridgeValidatorsContract =
    isRewardableBridge ? RewardableValidators : BridgeValidators
  ...
  await initializeValidators({
    ...
    isRewardableBridge: isRewardableBridge,
  ...
  console.log('\n[Home] Get Bridgeable token')
  const erc677token = getContract(ERC677BridgeToken, ERC677_TOKEN_ADDRESS, { network: "home" })
